...
public class VerClientes extends JFrame {
    ...
    private Object[][] data;
    private String[] columnaNombres = {"Codigo", "Nombre", "Contacto"};

    public VerClientes(final MotorBaseDatos BaseDatos) {
        super("Listado de Clientes");
        
        this.BaseDatos = BaseDatos;

        try{
		data = BaseDatos.listaTabla("Clientes",3);
        } catch (Exception e){
		...
		ImageIcon imgTop = ClaseGlobal.crearIcono("iconos/topClientes.jpg");
        ...
        toolNuevo.addActionListener(new ActionListener(){
			public void actionPerformed(ActionEvent e){
				new ControlCliente(VerClientes.this, BaseDatos, false, null);
			}
		});
        ...
        toolModificar.addActionListener(new ActionListener(){
			public void actionPerformed(ActionEvent e){
				if(codigoSeleccionado != null)
					new ControlCliente(VerClientes.this, BaseDatos, false, codigoSeleccionado);
			}
		});
        ...
        toolInformacion.addActionListener(new ActionListener(){
			public void actionPerformed(ActionEvent e){
				if(codigoSeleccionado != null)
					new ControlCliente(VerClientes.this, BaseDatos, true, codigoSeleccionado);
			}
		});
		...
		toolEliminar.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent e) {
				if(codigoSeleccionado != null && selectedRow != -1){
					int n = JOptionPane.showConfirmDialog(VerClientes.this, "\u00bfEsta seguro que desea eliminar el registro seleccionado?","Confirmar eliminación permanente",JOptionPane.YES_NO_OPTION);		
					if(n == 0)
						if(BaseDatos.eliminar("Clientes","IdCliente="+codigoSeleccionado))
							tableModel.removeRow(selectedRow);
				} else {
					JOptionPane.showMessageDialog(VerClientes.this,"Para continuar seleccione un registro de la lista.","Seleccione registro",JOptionPane.INFORMATION_MESSAGE);
				}
			}
		});
