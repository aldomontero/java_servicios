...
public class VerEquipos extends JFrame {
    ...
    private Object[][] data;
    private String[] columnaNombres = {"Codigo", "Cliente", "Marca", "Modelo", "No. serie", "No. inventario"};

    public VerEquipos(final MotorBaseDatos BaseDatos) {
        super("Listado de Equipos");
        
        this.BaseDatos = BaseDatos;
        try{
		data = BaseDatos.listaTabla("Equipos",6);
        }catch(Exception e){
		...
		ImageIcon imgTop = ClaseGlobal.crearIcono("iconos/topEquipos.jpg");
        ...
        toolNuevo.addActionListener(new ActionListener(){
			public void actionPerformed(ActionEvent e){
				new ControlEquipos(VerEquipos.this, BaseDatos, false, null);
			}
		});
        ...
        toolModificar.addActionListener(new ActionListener(){
			public void actionPerformed(ActionEvent e){
				new ControlEquipos(VerEquipos.this, BaseDatos, false, codigoSeleccionado);
			}
		});
        
        toolInformacion.addActionListener(new ActionListener(){
			public void actionPerformed(ActionEvent e){
				new ControlEquipos(VerEquipos.this, BaseDatos, true, codigoSeleccionado);
			}
		});
		...
		toolEliminar.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent e) {
				if(codigoSeleccionado != null && selectedRow != -1){
					int n = JOptionPane.showConfirmDialog(VerEquipos.this, "\u00bfEsta seguro que desea eliminar el registro seleccionado?","Confirmar eliminación permanente",JOptionPane.YES_NO_OPTION);		
					if(n == 0)
						if(BaseDatos.eliminar("Equipos","IdEquipo="+codigoSeleccionado))
							tableModel.removeRow(selectedRow);
				} else {
					JOptionPane.showMessageDialog(VerEquipos.this,"Para continuar seleccione un registro de la lista.","Seleccione registro",JOptionPane.INFORMATION_MESSAGE);
				}
			}
		});
